...
def _platform_is_emscripten() -> bool:
    ...
    if sys.platform in ("emscripten", "wasi") or sys.platform.startswith(("emscripten", "wasm")):
        return True
    if hasattr(sys, "_emscripten_info"):
        return True
    if "pyodide" in sys.modules or "pyscript" in sys.modules or "js" in sys.modules:
        return True
    try:
...
async def _ensure_micropip():
    ...
    try:
        ...
        if hasattr(pyodide, "loadPackage"):
    ...
    try:
        from js import pyodide  # type: ignore
        await pyodide.loadPackage("micropip")
        import micropip  # type: ignore
        return micropip
    except Exception:
    ...
    if not _platform_is_emscripten():
        raise RuntimeError("micropip is only available in Emscripten/Pyodide environment")
...
async def _platform_install(
    ...
    *,
    ...
    if _platform_is_emscripten():
        ...
        await mp.install(requirements, **kwargs)

        if constraint_lines and hasattr(mp, "set_constraints"):
            try:
                mp.set_constraints(constraint_lines)
            except Exception:
